                        ...
                        CoursesData.map(({topic, courses, id, def, time, starts, duration})=>(
                            <div className="grid grid-cols-3 group">
                                ...
                                <div className={`absolute pb-12 shadow-lg min-h-[430px] bg-white left-56 top-0 right-0  ${def ? ('default:inline-block') : ('hidden')} group-hover:inline-block text-black`}>
                                    ...
                                    <div className="grid grid-cols-2 xl:grid-cols-3 gap-x-5 gap-y-5 px-5">
                                        {courses.map(({title, list})=>(
                                            <div>
                                                <h1 className="text-lg mb-2 text-center">{title}</h1>
                                    ...
                                    <div className="absolute pb-2 pt-1 border border-t-gray-300 bottom-0 left-0 right-0 flex justify-around text-gray-500 text-xl font-semibold">
                                        {time && <h1><span className="text-red-600 xl:text-3xl">{time}</span> Hours</h1>}
                                        <h1>Batch Starts: <span className="text-green-600 xl:text-2xl">{starts}</span></h1>
                                        <h1>{duration}</h1>
                                    </div>
                                </div>
